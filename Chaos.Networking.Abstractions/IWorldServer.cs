...
public interface IWorldServer<in TClient> : IServer<TClient> where TClient: ISocketClient
{
    ...
    /// </summary>
    ValueTask OnBeginChant(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client accesses a board or mail
    /// </summary>
    ValueTask OnBoardRequest(TClient client, in ClientPacket clientPacket);
    ...
    /// </summary>
    ValueTask OnClick(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client is redirected to this server
    /// </summary>
    ValueTask OnClientRedirected(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client walks in a direction
    /// </summary>
    ValueTask OnClientWalk(TClient client, in ClientPacket clientPacket);
    ...
    /// </summary>
    ValueTask OnExchange(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client tries to log out
    /// </summary>
    ValueTask OnExitRequest(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client drops gold on the ground
    /// </summary>
    ValueTask OnGoldDropped(TClient client, in ClientPacket clientPacket);
    ...
    /// </summary>
    ValueTask OnIgnore(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client drops an item on the ground
    /// </summary>
    ValueTask OnItemDropped(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client drops an item on a creature
    /// </summary>
    ValueTask OnItemDroppedOnCreature(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client requests tile data for the current map
    /// </summary>
    ValueTask OnMapDataRequest(TClient client, in ClientPacket clientPacket);
    ...
    /// </summary>
    ValueTask OnPickup(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client responds to a request for profile data (portrait, text)
    /// </summary>
    ValueTask OnProfile(TClient client, in ClientPacket clientPacket);
    
    /// <summary>
    ///     Occurs when a client requests the profile of another player
    /// </summary>
    ValueTask OnProfileRequest(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client sends a publicly visible message
    /// </summary>
    ValueTask OnPublicMessage(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client responds to a merchant menu
    /// </summary>
    ValueTask OnPursuitRequest(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client requests to raise a stat
    /// </summary>
    ValueTask OnRaiseStat(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client refreshes their viewport
    /// </summary>
    ValueTask OnRefreshRequest(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client changes their social status
    /// </summary>
    ValueTask OnSocialStatus(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client presses their spacebar
    /// </summary>
    ValueTask OnSpacebar(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client swaps two panel objects
    /// </summary>
    ValueTask OnSwapSlot(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client toggles their group availability
    /// </summary>
    ValueTask OnToggleGroup(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client changes their character's direction
    /// </summary>
    ValueTask OnTurn(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client unequips an item
    /// </summary>
    ValueTask OnUnequip(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client uses an item
    /// </summary>
    ValueTask OnUseItem(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client toggles a user option
    /// </summary>
    ValueTask OnUserOptionToggle(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client uses a skill
    /// </summary>
    ValueTask OnUseSkill(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client uses a spell
    /// </summary>
    ValueTask OnUseSpell(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client sends a private message to the server
    /// </summary>
    ValueTask OnWhisper(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client requests a list of all online players
    /// </summary>
    ValueTask OnWorldListRequest(TClient client, in ClientPacket clientPacket);

    /// <summary>
    ///     Occurs when a client clicks on a world map node
    /// </summary>
    ValueTask OnWorldMapClick(TClient client, in ClientPacket clientPacket);
}